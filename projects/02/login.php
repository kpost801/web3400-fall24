...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    //check to see if the user exits in the database
    $stmt = $pdo->prepare("SELECT * FROM `users` WHERE `email` = ?");
    $stmt->execute([$email]);
    $user = $stmt->fetch();

    //vars used to check the activations status of the user account
    $acttivation_code = $user['activation_code'];
    $full_name = $user['full_name'];

    //set the activation status for the user
    $accountActivated = substr($acttivation_code, 0, 9) === 'activated' ? true : false;

    // if user account exists and is activated and password is verified then log them in
    if($user && $accountActivated && password_verify($password, $user['pass_hash'])){
        // update last login date/time stamp
        ...
        // redirect the user to the profile page or admin dashboard base on their role
        if ($user['role'] == 'admin') {
